...
func (p Subscribe) PreHandle(r wsmanage.Request) error {
	err := middleware.WsJWTMiddleware(r)
	return err
}

func (p Subscribe) Handle(r wsmanage.Request) {
	data := r.GetData()
	c := r.GetConnection()
	...
	answer, err := Answer(c, userId.(string), string(message.FriendId), jsep, false)
	if err != nil {
